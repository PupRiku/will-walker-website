...
const VALID_CATEGORIES = [
  'Drama', 'Comedy', 'Historical Drama', "Children's Play", 'Political Satire',
  'Thriller', 'SciFi/Fantasy', 'Radio Play', 'One Act Play', 'Screenplay',
  'Comedy/Drama', 'Theater for Youth', 'Collection',
]
...
export async function POST(request: Request) {
  ...
  try {
    ...
    if ((e as { code?: string }).code === 'P2002') {
      return NextResponse.json({ error: 'A play with this slug already exists' }, { status: 400 })
    }
    return NextResponse.json({ error: 'Internal server error' }, { status: 500 })
  }
}
