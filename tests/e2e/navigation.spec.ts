...
test.describe('Navigation and routing', () => {
  ...
  test('clicking "Selected Works" nav link scrolls to #plays section', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('link', { name: /selected works/i }).first().click();
    await expect(page).toHaveURL('/#plays');
  });

  test('clicking "Productions" nav link navigates to /productions', async ({ page }) => {
    await page.goto('/');
    await page.getByRole('link', { name: /^productions$/i }).first().click();
  ...
  test('clicking "Contact Me" button scrolls to #contact section', async ({ page }) => {
    await page.goto('/');
  ...
  test('logo click navigates to /#home', async ({ page }) => {
    await page.goto('/cv');
    await page.getByRole('link', { name: /william l\. walker montgomerie/i }).first().click();
    await expect(page).toHaveURL(/\/#home/);
  });
});
